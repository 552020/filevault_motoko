...
persistent actor Filevault {
  ...
  // Verify Privy JWT via HTTPS outcall to microservice
  public shared func verifyPrivyJWT(token : Text) : async Text {
    let url = "http://localhost:3000/verify"; // Change to your deployed URL as needed

    let bodyText = "{\"token\": \"" # token # "\"}";
    let bodyBlob = Text.encodeUtf8(bodyText);

    let headers : [Http.HeaderField] = [
      ("Content-Type", "application/json"),
      ("Content-Length", Nat.toText(bodyBlob.size()))
    ];
    ...
    switch (response.status_code) {
      case 200 {
        ...
        switch (jsonText) {
          case null { return "Failed to decode JSON response"; };
          case (?text) {
            Debug.print("Verification response: " # text);
            if (Text.contains(text, "\"success\":true")) {
              return "JWT Verified Successfully";
            } else {
              return "JWT Verification Failed: " # text;
            };
          };
        };
      };
      case _ {
        return "Microservice Error: HTTP " # Nat.toText(response.status_code);
      };
    };
  };

  // Example test function
  public shared func testJWT(token : Text) : async Text {
